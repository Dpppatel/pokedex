...
const Quiz = ({pokedex, selection}) => {
    ...
    const change = (event) =>{
        ...
        if(list.find(el => el.name === event.target.value.toLowerCase()) && !track.includes(event.target.value)){
    ...
    <div className="quiz">
        ...
        <div className="pokemonsImage">{list.map((el,i) => (<img key={i} src={`https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/${el.url.split('/').slice(-2, -1).slice(0)}.png`} />))}
        </div>
    </div>)
}
